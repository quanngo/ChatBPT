...
import { AddGrMemberToGroupbyPickCode, GetInfoResident, AddChatGroup, GetImageResidentbyUserID, AddGrMember, GetInfoUserbyUserId } from '../../../APIs/APIclass';
...
export default class CreatChat extends Component {
  ...
  submit() {
    if (this.state.Title.trim() == '' || this.state.filePath == {}) {
      ...
      let membs = this.state.selectedItems.toString();

      AddChatGroup(this.state.Title, 2, UserID, this.state.filePath.data, 1, BuildingID, "", 1, PickCode)
        .then(res => {
          if (res.toString() !== "false") {
            AddGrMemberToGroupbyPickCode(PickCode, UserID + ',' + membs.toString())
              .then(response => {
