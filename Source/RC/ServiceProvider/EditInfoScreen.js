...
import { KeyboardAvoidingView, StyleSheet, Image, Button, Text, View, Vibration, TextInput, TouchableOpacity, TouchableWithoutFeedback, Keyboard } from 'react-native';
import ImagePicker from 'react-native-image-picker';
import { UpdateUserInfo, UpdateProviderService } from '../../../APIs/APIclass';
...
export default class EditPost extends React.Component {
  ...
  componentDidMount() {
    ...
    const Providerlevel = navigation.getParam('Providerlevel');
  ...
  submit() {
    if (this.state.NameService.trim() == '' || this.state.Description.trim() == '') {
      ...
      if (this.state.filePath = {}) {
        UpdateUserInfo(UserID, Email, this.state.PhoneNumber, this.state.NameInfo, Token)
          .then(response => {
            if (response.toString() == "true") {
              UpdateProviderService(UserID, this.state.NameService, this.state.Description, CategoryID, Providerlevel, this.state.Avatar)
                .then(res => {
        ...
        UpdateUserInfo(UserID, Email, this.state.PhoneNumber, this.state.NameInfo, Token)
          .then(response => {
            if (response.toString() == "true") {
              UpdateProviderService(UserID, this.state.NameService, this.state.Description, CategoryID, Providerlevel, this.state.filePath.data)
                .then(res => {
                  if (res.toString() == "true") {
                    alert("Cập nhật thành công!");
                    this.props.navigation.goBack();
      ...
      <KeyboardAvoidingView
        ...
        behavior="padding" enabled>
          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={{ top: -50, flex: 1, justifyContent: "flex-end", }}  >
              ...
              <View style={styles.textInputContainer}>
                <TextInput style={styles.textInput}
                  onChangeText={(text) => this.updateValue(text, 'NameService')}
                  value={this.state.NameService.toString()}>
                </TextInput>
              </View>
              <Text>
                Mô tả dịch vụ:
